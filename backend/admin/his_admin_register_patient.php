...
<?php
    ...
    if (isset($_POST['add_patient'])) {
        ...
        $pat_number = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 1, 5); // Generate unique patient number
        $pat_phone = $_POST['pat_phone'];
        $pat_type = $_POST['pat_type'];
        $pat_addr = $_POST['pat_addr'];
        $pat_age = $_POST['pat_age'];
        $pat_dob = date("Y-m-d", strtotime($_POST['pat_dob'])); // Convert date to SQL format
        $pat_ailment = $_POST['pat_ailment'];
    
        // Prepare the statement to call the stored procedure
        $query = "CALL add_patient_details(?, ?, ?, ?, ?, ?, ?, ?, ?)";
