...
----PROCEDURES-----
 ...
 /* 
    This procedure, named `add_patient_details`, is designed to simplify the process of inserting patient details into the 
    `his_patients` table. It accepts several input parameters, including the patient's first name, last name, unique number, 
    phone number, type (e.g., outpatient or inpatient), address, age, date of birth, and ailment. Once invoked, the procedure 
    executes an `INSERT` statement that populates the corresponding fields in the table with the provided values. This approach 
    ensures consistent and efficient data entry for new patients in the hospital information system. 
    */

DELIMITER //

CREATE PROCEDURE add_patient_details(
    IN pat_fname VARCHAR(50),
    IN pat_lname VARCHAR(50),
    IN pat_number VARCHAR(10),
    IN pat_phone VARCHAR(15),
    IN pat_type VARCHAR(10),
    IN pat_addr VARCHAR(100),
    IN pat_age INT,
    IN pat_dob DATE,
    IN pat_ailment VARCHAR(100)
)
BEGIN
...
/*
This query retrieves comprehensive patient details by combining data from multiple tables in the hospital database system. 
It provides information about the patient's basic details, recent vital statistics, last prescription details, and 
historical medical records. Here's a detailed explanation:

1. **Patient Details (from `his_patients` table)**:
   - `pat_id`, `pat_fname`, and `pat_lname` provide unique identifiers and name details of the patient.
   - `pat_number` is the unique patient number used to join with other tables.
   - `pat_addr` and `pat_phone` provide the patient's contact details.
   - `pat_age` gives the patient's age, and `pat_type` specifies whether the patient is an inpatient or outpatient.

2. **Recent Vitals (from `his_vitals` table)**:
   - Retrieves the most recent body temperature, heart pulse rate, respiratory rate, and blood pressure.

3. **Prescription Information (from `his_prescriptions` table)**:
   - Captures the date of the last prescription and the ailment associated with it.

4. **Medical Records (from `his_medical_records` table)**:
   - Includes the patient's historical ailment details and the corresponding record date.

5. **Joins**:
   - `LEFT JOIN` ensures that all patients are listed, even if some data (e.g., vitals, prescriptions, or medical records) 
     is missing.

6. **Sorting**:
   - Orders the results alphabetically by the patient's first and last names for improved readability.

This query provides a holistic view of a patient by aggregating data from multiple related tables.
*/
